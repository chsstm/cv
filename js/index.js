function basicInfo() {
  $.ajax({
    ...
    success: function (result) {
      ...
      $("#avatar-div").html(
        "<img src='images/" + info.avatar + "' style='width : 100%;'/>"
      );
      $("#profile-name").html(info.name);
      $("#basic-info-div").append(
        '<p><i class="fa fa-laptop fa-fw w3-margin-right w3-large w3-text-teal"></i>' +
          info.work +
          "</p>"
      );
      $("#basic-info-div").append(
        "<p><i class='fa fa-briefcase fa-fw w3-margin-right w3-large w3-text-teal'></i>" +
          ...
          "<img src='images/" +
          info.companyLogo +
          "' style='margin-left: 10px; width : 100px; object-fit: contain;'/></p>"
      );
      $("#basic-info-div").append(
      ...
      for (var i = 0; i < info.mobiles.length; i++) {
        var value =
          '<p><i class="fa fa-volume-control-phone fa-fw w3-margin-right w3-large w3-text-teal"></i><span class="flag-icon flag-icon-' +
          info.mobiles[i].country +
          '"></span> ' +
          info.mobiles[i].number +
          "</p>";
    ...
    success: function (result) {
      ...
      for (var i = 0; i < info.length; i++) {
        var value =
          ...
          '<div class="w3-container w3-center w3-round-xlarge w3-teal" style="font-size: 14px; line-height: 22px; height:22px; width:' +
          info[i].percent +
          '%">' +
          info[i].language +
          "</div>" +
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      for (var i = 0; i < info.length; i++) {
        ...
        var duration = moment
          .duration(
            (info[i].current == true ? moment() : moment(info[i].end)).diff(
              moment(info[i].start),
              "months"
            ),
            "months"
        ...
        var value =
          ...
          info[i].role +
          "</b></h5>" +
          '<h6 class="w3-text-teal"><i class="fa fa-code fa-fw w3-margin-right"></i>' +
          info[i].skills +
          "</h6>" +
          '<h6 class="w3-text-teal"><i class="fa fa-building fa-fw w3-margin-right"></i>' +
          info[i].company +
          "</h6>" +
          ...
          duration +
          ")</h6>" +
          '<p><i class="fa fa-quote-left fa-fw w3-text-teal w3-margin-right"></i>' +
          info[i].description +
          "</p>" +
    ...
    success: function (result) {
      ...
      for (var i = 0; i < project.length; i++) {
        ...
        if (project[i].running == true) {
          duration =
            project[i].duration +
            ' - <span class="w3-tag w3-teal w3-round">Running</span>';
        }
        if (project[i].link != "") {
          link =
            '<a target="new" href="' +
            project[i].link +
            '"><h6 class="w3-text-blue"><i class="fa fa-chain fa-fw w3-margin-right"></i>' +
            project[i].link +
            "</h6></a>";
        }
    ...
    success: function (result) {
      ...
      for (var i = 0; i < info.length; i++) {
        ...
        var value =
          ...
          info[i].level +
          "</b></h6>" +
          '<p class="w3-text-teal"><b>' +
          info[i].trade +
          "</b></p>" +
          '<h6 class="w3-text-teal"><i class="fa fa-calendar fa-fw w3-margin-right"></i>' +
          (info[i].year ? info[i].year : info[i].start + " - " + info[i].end) +
          "</h6>" +
          '<p><i class="fa fa-institution w3-text-teal fa-fw w3-margin-right"></i>' +
          info[i].college +
          "</p>" +
          '<hr style="border-color:' +
          hrColor +
    ...
    success: function (result) {
      ...
      for (var i = 0; i < info.length; i++) {
        var value =
          '<a href="' +
          info[i].link +
          '" target="_blank"> &nbsp; <i class="fa s-m-i fa-3x fa-' +
          info[i].icon +
